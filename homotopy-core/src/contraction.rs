...
fn collapse_base<'a, Ix: IndexType>(
    ...
    let mut sums = HashMap::<Vec<Height>, isize>::default();
    ...
    for (i, (d, _bias, coord)) in graph.node_references() {
        ...
        if g.dimension == max_dim_generator.dimension {
            ...
            *sums.entry(key).or_default() += g.orientation;
        }
    }
    ...
    // check orientations
    let (_, &first_sum) = sums.iter().next().unwrap();
    if !(first_sum <= 1 && first_sum >= -1) {
        return Err(ContractionError::Invalid);
    }
    for (_, sum) in sums {
        if sum != first_sum {
